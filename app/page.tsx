...
export default function VotingPage() {
  ...
  async function handleLogout() {
    try {
      ...
      setError(`שגיאה בהתנתקות: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  async function loadData() {
    try {
      ...
      setError(`שגיאה בטעינת נתונים: ${err instanceof Error ? err.message : String(err)}`)
      setLoading(false)
    }
  }
  ...
  function handlePredictionChange(candidateName: string, value: string) {
    try {
      ...
      setError(`שגיאה בעדכון ערך: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  async function handleSubmit() {
    try {
      ...
      setError(`שגיאה בשליחה: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  async function submitVote(userId: string) {
    try {
      ...
      if (error) {
        setError(`שגיאה בשמירת ההצבעה: ${error.message}`)
        console.error(error)
      } else {
      ...
      setSubmitting(false)
      setError(`שגיאה בשמירת הצבעה: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
  ...
  async function handleAuthSuccess() {
    try {
      ...
      setError(`שגיאה באימות: ${err instanceof Error ? err.message : String(err)}`)
    }
  }
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {error && (
            <div className="mb-4 p-4 bg-red-100 border border-red-400 text-red-700 rounded-lg">
              ...
              <p className="text-sm mt-1 whitespace-pre-wrap break-words">{error}</p>
              <button
